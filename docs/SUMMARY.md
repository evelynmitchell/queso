# Summary

<!-- This file is mdbook's table of contents for the published docs site
     (issue #80) -- it gives the files in this directory an order and a
     sidebar, nothing more. Reading the .md files directly on GitHub
     remains fully supported; the README stays the canonical front door
     and lists the same files with descriptions. -->

- [Project outline](00-project-outline.md)
- [Backgrounder: the consensus problem space](01-backgrounder.md)
- [Property model](02-properties.md)
- [Testing plan](03-testing-plan.md)
- [What each test establishes](what-each-test-establishes.md)
- [Investigating with logs](investigating-with-logs.md)
- [TLA+ models (spec/)](spec.md)
- [Status & gap analysis](STATUS.md)
- [Comparing against etcd](compare-etcd.md)
- [Deploying to fly.io](deploy-flyio.md)

---

- [Blog: Queso vs the Antithesis Raft bug hunt](blog/2026-antithesis-raft-and-queso.md)
